...
#This version is update past the completion of the STAT 3553 Course
#Sections Updated are the following:
# - PCA algorithm overhaul
...
par(mfrow=c(2,2))
plot(mod_3)#We can observe that the model is linear, but normality is not satisfied.
...
design_matrix2 <- model.matrix(~ `Avg. Landing Force`+`Countermovement Depth`+`Impulse Ratio`+`Peak Relative Propulsive Power`+`Relative Peak Landing Force`+`Right Avg. Braking RFD`+`Unweighting Phase %`, data = data5)
...
summary(mod_7) #Mod 4 is the final model, as it is the most robust and meets all criteria for hypothesis testing.
par(mfrow=c(2,2))
plot(mod_7)

mod_null1<-lm(`Jump Height`~-sex-sport+.,data=data5)
anova(mod_null1,mod_7) #ANOVA F-Test rejects when comparing model with no sex interaction to model with sex categorical variable interactions.

testPlayer<-data5[2000,] #Test player

predictedJumpHeight<-predict(mod_7,testPlayer)

testPlayerBBall<-testPlayer
testPlayerBBall$sport<-"basketball"
predictedJumpHeightBasketball<-predict(mod_7,testPlayerBBall)

testPlayerBBallM<-testPlayer
testPlayerBBallM$sex<-"m"
predictedJumpHeightBasketballM<-predict(mod_7,testPlayerBBallM)
